...
<head>
	...
	<style>
		...
		#boss {
			...
			background-image: url("/img/boss1.png");
		}
	</style>
</head>
...
<body>
	<div id="con">
		<div class="won" style="color:white;"><h1>You Won!</h1></div>
		<div class="atk">
			<h1>Attacked</h1>
		</div>
	</div>
...
<script>
	...
	atk.on('child_added', snap => {
		console.log(snap.val().item);
		var id = snap.val().item;
		if (id == 'connected') {
			Materialize.toast('Connected wating for command', 1000);
		} 
		else if (id == 'phasechange'){
			$("#boss").show();
			$("#boss").css("background-image","url('/img/boss2.png')");
			$(".won").hide();
		}
		else if (id == 'won'){
			$("#boss").fadeOut(400);
	...
	$(document).ready(function () {
		$("#boss").css("background-image","url('/img/boss1.png')");
		$(".atk").hide();
		$(".won").hide();
	});
	/*
